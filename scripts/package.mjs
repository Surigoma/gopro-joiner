...
const backendName = process.platform === "win32" ? "gopro-joiner-backend.exe" : "gopro-joiner-backend";
...
if (process.platform === "darwin") {
  ...
  renameSync(original, path.join(target, process.platform === "win32" ? "GoPro Joiner.exe" : "gopro-joiner"));
...
const status = spawnSync(backend, [], {
  ...
  shell: false,
  windowsHide: true,
  env: { ...process.env, GOPRO_JOINER_TOOLS_DIR: "" }
});
...
writeFileSync(manifestPath, `${JSON.stringify({ application: "GoPro Joiner", version: packageJson.version, platform: process.platform, arch: process.arch, artifacts, nativeTools }, null, 2)}\n`);
